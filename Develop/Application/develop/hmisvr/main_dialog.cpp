...
void main_dialog::init_database()
{
    ...
    if(read_profile_string(Section_name, Key_name, Tempor_name, INIFILE_LININBUF_SIZE,"",config_path))
    {
        ...
        char code_appdist[40] = {"/appdownload/?charging_pileCode="};
        char code_gun   = '1';

        snprintf(buf_code,120,"/usr/APP/qrencode -o 1.png -s 4 %s%s%s%c",websize,code_appdist,code,code_gun);

        //snprintf(buf_code,100,"/usr/APP/qrencode -o 1.png -s 6 %s",code);
  ...
  if(language_num == 1)
  {
      ...
      if(service_open == 1)
      {
        ui->label_title_3->setText(QString::fromUtf8("请  扫  码  充  电"));
      }
...
void main_dialog::check_hmi_page_index()
{
   ...
   //     printf("@@@@Go to PAGEINDEX_SETPASSWRDIPT!\r\n");
    ...
    if(hmi_page_index==PAGEINDEX_CONFIRM)
    {
    //    printf("@@@@Go to PAGEINDEX_CONFIRM!\r\n");
        call_p21_dlg();
    }else
